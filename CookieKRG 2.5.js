// CookieKRG 2.5 - Supreme Brain Logic (Refined Stability)
const internalBrain = [
    {
...
// v2.5 Optimized KRG Connection
async function getGlobalKnowledge(query) {
    status.innerText = "Syncing KRG Archives...";
    status.classList.add('text-sky-500');
    
    // v2.5 Controller: Prevent infinite loading
...
form.addEventListener('submit', async (e) => {
    ...
    // Stage 2: Identity Logic (v2.5 Release Name)
    if (lowerText.includes("your name") || lowerText.includes("who are you")) {
        setTimeout(() => {
            appendMessage("I am the CookieKRG 2.5, the core executor of this Supreme Brain system there actually also! 👋", false);
        }, 300);
        return;
    }
